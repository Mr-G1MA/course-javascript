...
const server = http.createServer(async (req, res) => {
  try {
    if (/\/photos\/.+\.png/.test(req.url)) {
      ...
      const fallBackPath = path.resolve(__dirname, '../img/default_photo.png');
      ...
      if (name && content) {
        ...
        broadcast(connections, { type: 'new-avatar', data: { name } });
      } else {
        return res.end('fail');
      }
    }
...
wss.on('connection', (socket) => {
  ...
  socket.on('close', () => {
    sendMessageFrom(connections, { type: 'leave' }, socket);
